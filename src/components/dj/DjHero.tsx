    ...
    <section className="relative flex flex-col items-center justify-center min-h-screen px-6 pt-20 overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 flex items-end justify-center w-full h-64 gap-2 opacity-20 -z-10">
        {[...Array(20)].map((_, i) => (
          <motion.div
            ...
            className="w-8 rounded-t-sm bg-linear-to-t from-neon-pink to-neon-purple"
          />
        ))}
      </div>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-6 sm:flex-row">
          <button
            className="w-full px-10 py-5 text-xl font-bold tracking-widest text-white transition-all duration-300 rounded-full sm:w-auto bg-neon-pink font-display hover:bg-white hover:text-neon-pink neon-box-pink"
            onClick={() => window.scrollTo(0, 3400)}
          >
            BOOK A SET
          </button>
          <button
            className="flex items-center justify-center w-full gap-3 px-10 py-5 text-xl font-bold tracking-widest transition-all duration-300 border-2 rounded-full sm:w-auto border-neon-pink text-neon-pink font-display hover:bg-neon-pink/20"
